...
def add_mail_from_backup_db(main_db: Database, input_file: str) -> Database:
    ...
    main_db.copy_value_of_property_from_referencelist_if_empty_and_all_other_properties_match(
        "email", backup_db.people
    )
    return main_db
...
def remove_mail_from_removelist(main_db: Database, input_file: str) -> Database:
    ...
    remove_from_mailinglist_db_only_mail = [
        Person(emails=[person.email]) for person in remove_from_mailinglist_db.people
    ]
    main_db.remove_property_for_people_matching_removelist(
        "email", remove_from_mailinglist_db_only_mail
    )
    return main_db
...
# GV
export_adult_people_joined_at_or_after_date_excel(main_db, pd.Timestamp('2023-12-01'), path+fout_neumitglieder)
