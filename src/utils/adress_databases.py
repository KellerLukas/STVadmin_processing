...
class HouseBasedAdressDatabase:
    ...
    def __create_from_house_based_database(self, hb_db: HouseBasedDatabase):
        ...
        self._df.sort_values(by=["PLZ", "Ort", "Strasse", "Name"], inplace=True, ignore_index=True)
    ...
    def __add_family_entry(self, hbfamily: HouseBasedFamily):
        ...
        if len(hbfamily.people) > 1:
            ...
            entry_constructor_dict["Name"] = f"{hbfamily.people[0].first_name} {hbfamily.people[0].last_name}"
        ...
        assert set(entry_constructor_dict.keys()) == set(self.columns)
        self._df = pd.concat([self._df, pd.DataFrame.from_dict(entry_constructor_dict)], ignore_index=True)
