...
class TestCleverreachDatabase(TestCase):
    def test_load_cleverreach_database(self):
        ...
        expected_df = pd.read_csv(
            "tests/data/test_cleverreach_database_expected_output.csv"
        )

        for col in expected_df.columns:
            if col == "updated":
                expected_df[col] = [
                    pd.Timestamp.now().floor(freq="D").strftime("%d.%m.%Y")
                    for _ in expected_df[col].values
                ]
            if col == "Email":
                expected_df[col] = [
                    val if not isinstance(val, type(np.nan)) else None
                    for val in expected_df[col].values
                ]
